  ...
  Future<String> _verifyOtpAndLogin() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        showDelightfulToast(
            context, response.data["message"].toString(), false);
        return "0";
      } else if (response.statusCode == 401) {
        showDelightfulToast(context, "Unauthorized access. Warning.", false);
        return "0";
      } else if (response.statusCode == 400) {
        if (kDebugMode) {
          print(response);
        }
        showDelightfulToast(context, "Invalid OTP. Please try again!", false);
      } else {
        showDelightfulToast(context, "Something went wrong.", false);
        return "0";
      }
    } catch (e) {
      if (kDebugMode) {
